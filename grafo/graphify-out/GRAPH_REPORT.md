...
1. `AplicacionPrincipal` - 85 edges
...
Cohesion: 0.12
Nodes (33): actualizar_estado_asignado(), actualizar_oficio(), _exigir_datos_para_finalizar(), _exigir_no_anulado(), _FilaRepetida, filtrar_oficios(), _preparar_importado(), Filtra una lista de oficios. Todos los filtros se acumulan (Y lógico).      - `c (+25 more)
...
Cohesion: 0.07
Nodes (45): Bloqueo entre procesos para la carpeta de datos compartida.  Problema que resuel, _cifrador(), cifrar(), descifrar(), obtener_clave(), Devuelve la clave Fernet; la genera la primera vez que se ejecuta., Descifra. Lanza cryptography.fernet.InvalidToken si el archivo     fue alterado, _buscar_recurso() (+37 more)
...
Cohesion: 0.10
Nodes (12): Panel de búsqueda. Tres bloques que se acumulan entre sí:          - por texto,, Repuebla los desplegables de filtro conservando lo elegido.          El tipo de, Precarga el panel de edición con los datos del oficio seleccionado.          Sol, Ancho de corte de las etiquetas de los formularios.          El corte se fija en, Tablero con scroll vertical: tarjetas de indicadores y gráficos., Texto que se muestra en los desplegables para un responsable.         Incluye el, Personas a las que se les puede asignar un oficio.          Un administrador no, True si el usuario en sesión puede crear/editar/eliminar usuarios         y reas (+4 more)
...
Cohesion: 0.08
Nodes (11): AplicacionPrincipal, Ajusta cuántas filas muestra la tabla de oficios al alto disponible.          De, Barras verticales: oficios recibidos por día., Barras verticales: oficios recibidos por mes., Gráfico de anillo con la distribución por estado., Barras horizontales: cantidad de oficios por responsable., Marco superior con logo y título., Acorta el título de la cabecera cuando la ventana es estrecha.          `pack` n (+3 more)
...
### Community 11 - ".get"
Cohesion: 0.10
Nodes (10): Valor elegido en un desplegable de filtro, o '' si es "(Todos)"., Devuelve la clave interna a partir de la etiqueta mostrada., Guarda los cambios del panel según el rol: el gestor puede cambiar         respo, Sustituye el documento del oficio por si se cargó el equivocado., Carga un PDF con la respuesta del oficio seleccionado., Elimina el PDF adjunto (por si se cargó el archivo equivocado)., Repuebla los desplegables de tipo de acción tras cambiar el catálogo., Nombre del tipo elegido en la lista, sin el contador. (+2 more)
...
Cohesion: 0.11
Nodes (40): actualizar_implicado(), adjuntar_respuesta(), agregar_implicado(), anular_oficio(), contar_por_tipo_accion(), corregir_oficio(), eliminar_implicado(), eliminar_respuesta() (+32 more)
...
### Community 28 - "._aviso_sin_alcance"
Cohesion: 0.25
...
## Knowledge Gaps
- **46 isolated node(s):** `Dependencias`, `IMPORTANTE: actualizar el grafo tras CADA cambio de código`, `Qué se versiona y qué no`, `Consultar el grafo`, `Dependencias externas` (+41 more)
...
- **Why does `AplicacionPrincipal` connect `AplicacionPrincipal` to `aplicacion.py`, `.__init__`, `.get`, `._al_recuperar_foco`, `._construir_configuracion`, `._mostrar_pdf`, `._aviso_sin_alcance`, `._oficio_por_referencia`?**
